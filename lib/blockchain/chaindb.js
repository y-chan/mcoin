...
ChainDB.prototype.open = async function open() {
  ...
  // ## Mongo Note
  // Mongo is already open on init
  // We don't care about hardcoded versions, we can add to Meta
  // If needed.
  await this.db.open();
  await this.db.checkVersion('V', 3);

  // Mongo handles inside getState
...
ChainDB.prototype.getHeight = async function getHeight(hash) {
  ...
  const mHeight = await this.mongo.getBlockHeightByHash(hash);
...
ChainDB.prototype.getHash = async function getHash(height) {
  ...
  const mHash = await this.mongo.getBlockHashByHeight(height);
...
ChainDB.prototype.getEntryByHeight = async function getEntryByHeight(height) {
  ...
  const mData = await this.mongo.getBlockHashByHeight(height);
...
ChainDB.prototype.getEntryByHash = async function getEntryByHash(hash) {
  ...
  const mRaw = await this.mongo.getEntryByHash(hash);
...
ChainDB.prototype.getEntry = async function getEntry(block) {
  if (typeof block === 'number')
    // return await this.mongo.getEntryByHeight(block)
    return this.getEntryByHeight(block);
    // return await this.mongo.getEntryByHash(block)
...
ChainDB.prototype.getTip = function getTip() {
  // return this.mongo.getTipHash(hash);
...
ChainDB.prototype.checkDeployments = async function checkDeployments() {
  ...
  const mRaw = await this.mongo.getDeploymentBits();
...
ChainDB.prototype.prune = async function prune(tip) {
  ...
  try {
    ...
    // Mongo will update the flags, but we still need these writes until b & u are replaced.
    await batch.write();
  } catch (e) {
    options.prune = false;
    throw e;
...
ChainDB.prototype.unindexTX = async function unindexTX(tx, view) {
  ...
  if (this.options.indexTX) {
    ...
    await this.mongo.deleteTx(hash);
