...
Chain.prototype.getTarget = async function getTarget(time, prev) {
  const pow = this.network.pow;
  const spacing = 1.5 * 60; // 1.5 minutes
  ...
  if(prev.height + 1 >= hfLyraDGW) {
  	// DGWv3
  	return await this.darkGravityWave(time, prev);
  } else if(prev.height + 1 >= hfKGW && prev.height + 1 < hfDIGI) {
    // KGW
    return await this.kimotoGravityWell(time, prev, spacing, blocksMin, blocksMax);
  }

  let adjustInterval = pow.retargetInterval;
  if(prev.height + 1 >= hfDIGI) {
  	adjustInterval = pow.retargetIntervalDIGI;
  }
    
  // Do not retarget
  if ((prev.height + 1) % adjustInterval !== 0) {
    if (pow.targetReset) {
  ...
  // Back 2 weeks
  let back = adjustInterval - 1;
  if (prev.height + 1 !== adjustInterval)
    back = adjustInterval;
...
/**
 * Calculate difficulty using dark gravity wave formula.
 * @param {Number} time - Next block timestamp.
 * @param {ChainEntry} prev - Previous entry.
 */
Chain.prototype.darkGravityWave = async function darkGravityWave(time, prev) {
  const pow = this.network.pow;
  let actualTimespan = 0;
  let lastBlockTime = 0;
  let reading = prev;
  let pastBlocksMax = 24;
  let pastBlocksMin = 24;
  let countBlocks = 0;
  let difficultyAverage = consensus.fromCompact(0);
  let difficultyAveragePrev = consensus.fromCompact(0);

  if (!prev || prev.height === 0 || prev.height < hfLyraDGW + blocksMin) {
    return pow.bits;
  }

  for (let i = 1; reading && reading.height >= hfLyraDGW; i++) {
    ...
    if(countBlocks == 1) {
    	...
    	difficultyAverage = ((difficultyAveragePrev * countBlocks) + reading.bits) / (countBlocks + 1);
    }
    difficultyAveragePrev = difficultyAverage;

    if(lastBlockTime > 0) {
    	let diff = (lastBlockTime - reading.time);
    	ActualTimespan += diff;
    }
  ...
  let targetTimespan = countBlock * pow.targetSpacing;
...
Chain.prototype.retarget = function retarget(prev, first) {
  ...
  let newDifficultyProtocol = (prev.height + 1 >= hfDIGI);
  if(newDifficultyProtocol) {
  	targetTimespan = pow.targetTimespanDIGI;
  }
  ...
  if(newDifficultyProtocol) {
    ...
    if(actualTimespan < targetTimespan - (targetTimespan / 4 | 0))
      actualTimespan = targetTimespan - (targetTimespan / 4 | 0);
    if(actualTimespan < targetTimespan + (targetTimespan / 2 | 0))
      actualTimespan = targetTimespan + (targetTimespan / 2 | 0);
  } else {
  ...
  const bnPowLimit = pow.limit;

  let fShift = target > bnPowLimit - 1;
