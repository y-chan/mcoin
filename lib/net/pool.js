...
HostList.prototype.getHost = function getHost() {
  ...
  for (;;) {
    ...
    if (num < factor * entry.chance() * (1 << 30))
      return entry;
...
HostList.prototype.add = function add(addr, src) {
  var now = this.network.now();
  var penalty = 2 * 60 * 60; // default `addr` penalty
  ...
  if (entry) {
    ...
    if (addr.ts && entry.addr.ts < addr.ts - interval - penalty) {
      entry.ts = this.network.time.local(addr.ts);
      entry.addr.ts = addr.ts;
    }

    // Do not update if no new information is present.
    if (!addr.ts || (entry.addr.ts && addr.ts <= entry.addr.ts))
      return false;

    // Do not update if the entry was already in the "used" table.
    if (entry.used)
      return false;

    // Do not update if the max reference count is reached.
    // if (entry.refCount === this.newBucketsPerAddress)
    //   return false;
    ...
    if (factor > 1 && util.random(0, factor) !== 0)
      return false;
...
HostList.prototype.isStale = function isStale(entry) {
  ...
  if (entry.ts > now + 10 * 60)
    return true;

  if (entry.ts === 0)
    return true;

  if (now - entry.ts > this.horizonDays * 24 * 60 * 60)
    return true;
...
HostList.prototype.markSuccess = function markSuccess(hostname) {
  ...
  if (now > entry.ts + 60 * 20) {
    entry.ts = now;
    entry.addr.ts = this.network.time.adjust(now);
  }
};
...
function HostEntry(addr, src, network) {
  ...
  this.used = false;
  this.attempts = 0;
  this.ts = network.time.local(addr.ts);
  this.lastSuccess = 0;
  this.lastAttempt = 0;
}
...
HostEntry.prototype.chance = function _chance() {
  var now = util.now();
