 ...
 * @exports Transaction
 * @constructor
 * @param {NakedTransaction} data - Transaction fields.
...
function Transaction(data, block, index) {
  var i;

  if (!(this instanceof Transaction))
    return new Transaction(data, block, index);
...
Transaction.prototype.clone = function clone() {
  var copy, i;
...
Transaction.prototype.setBlock = function setBlock(block, index) {
  this.ts = block.ts;
...
Transaction.prototype.unsetBlock = function unsetBlock() {
  this.ts = 0;
  this.block = null;
  this.height = -1;
  this.index = -1;
...
Transaction.prototype.hash = function hash(enc) {
  if (!this._hash)
...
Transaction.prototype.witnessHash = function witnessHash(enc) {
  if (this.isCoinbase()) {
    return enc === 'hex'
...
Transaction.prototype.renderNormal = function renderNormal() {
  var raw = this.getRaw();
...
Transaction.prototype.renderWitness = function renderWitness() {
  var raw = this.getRaw();
  if (bcoin.protocol.parser.isWitnessTX(raw))
    return raw;
...
Transaction.prototype.getRaw = function getRaw() {
  var raw;
...
Transaction.prototype.getVirtualSize = function getVirtualSize() {
...
Transaction.prototype.getSize = function getSize() {
...
Transaction.prototype.hasWitness = function hasWitness() {
  var i;
...
Transaction.prototype.signatureHash = function signatureHash(index, prev, type, version) {
...
Transaction.prototype.signatureHashV0 = function signatureHashV0(index, prev, type) {
...
Transaction.prototype.signatureHashV1 = function signatureHashV1(index, prev, type) {
  var p = new BufferWriter();
...
Transaction.prototype.verify = function verify(index, force, flags) {
  var i, input, res;
...
Transaction.prototype.verifyAsync = function verifyAsync(index, force, flags, callback) {
  var self = this;
  var res;
...
Transaction.prototype.isCoinbase = function isCoinbase() {
...
Transaction.prototype.getFee = function getFee() {
  if (!this.hasCoins())
    return new bn(0);
...
Transaction.prototype.getInputValue = function getInputValue() {
  var total = new bn(0);
  var i;
...
Transaction.prototype.getOutputValue = function getOutputValue() {
  var total = new bn(0);
  var i;
...
Transaction.prototype.getInputAddresses = function getInputAddresses() {
  var table = {};
  var addresses = [];
  var i, address;
...
Transaction.prototype.getOutputAddresses = function getOutputAddresses() {
  var table = {};
  var addresses = [];
  var i, address;
...
Transaction.prototype.getAddresses = function getAddresses() {
...
Transaction.prototype.testInputs = function testInputs(addressMap, index) {
  var i;
...
Transaction.prototype.testOutputs = function testOutputs(addressMap, index) {
  var i;
...
Transaction.prototype.hasCoins = function hasCoins() {
  var i;
 ...
 * @param {Coin|Transaction|Coin[]|Transaction[]} coins
 * @returns {Boolean} Whether the transaction is now completely filled.
 */

Transaction.prototype.fillCoins = function fillCoins(coins) {
  var total = 0;
  ...
  if (Array.isArray(coins)) {
    coins = coins.reduce(function(out, coin) {
      if (coin instanceof Transaction) {
...
Transaction.prototype.isFinal = function isFinal(height, ts) {
...
Transaction.prototype._getSigops = function _getSigops(scriptHash, accurate) {
  var total = 0;
...
Transaction.prototype.getSigops = function getSigops(scriptHash, accurate) {
...
Transaction.prototype.isSane = function isSane(ret) {
  var prevout = {};
  var total = new bn(0);
...
Transaction.prototype.isStandard = function isStandard(flags, ret) {
  var i, input, output;
  var nulldata = 0;
...
Transaction.prototype.hasStandardInputs = function hasStandardInputs(flags) {
...
Transaction.prototype.checkInputs = function checkInputs(spendHeight, ret) {
  var total = new bn(0);
...
Transaction.prototype.maxSize = function maxSize() {
...
Transaction.prototype.getModifiedSize = function getModifiedSize(size) {
  var i, offset;
...
Transaction.prototype.getPriority = function getPriority(height, size) {
  var sum, i, input, age;
...
Transaction.prototype.isFree = function isFree(height, size) {
  var priority;
...
Transaction.prototype.getMinFee = function getMinFee(size) {
  var fee;
...
Transaction.prototype.getMaxFee = function getMaxFee(size) {
  var fee;
...
Transaction.prototype.getConfirmations = function getConfirmations(height) {
  if (height == null)
...
Transaction.prototype.getPrevout = function getPrevout() {
  var prevout = {};
  var i, input;
...
Transaction.prototype.isWatched = function isWatched(bloom) {
...
Transaction.prototype.__defineGetter__('rblock', function() {
  return this.block
    ? utils.revHex(this.block)
    : null;
});

Transaction.prototype.__defineGetter__('rhash', function() {
  return utils.revHex(this.hash('hex'));
});

Transaction.prototype.__defineGetter__('rwhash', function() {
  return utils.revHex(this.witnessHash('hex'));
});

Transaction.prototype.__defineGetter__('txid', function() {
  return this.rhash;
});

Transaction.prototype.__defineGetter__('wtxid', function() {
  return this.rwhash;
});
...
Transaction.prototype.inspect = function inspect() {
  return {
    type: this.type,
 ...
 * @param {Object} json
 * @returns {NakedTransaction} A "naked" transaction (a
 * plain javascript object which is suitable
 * for passing to the Transaction constructor).
 */

Transaction._fromJSON = function fromJSON(json) {
...
Transaction.fromJSON = function fromJSON(json) {
  assert.equal(json.type, 'tx');
  return new Transaction(Transaction._fromJSON(json));
};

/**
 * Serialize the transaction.
 * @see {Transaction#render}
 * @param {String?} enc - Encoding, can be `'hex'` or null.
 * @returns {Buffer|String}
 */

Transaction.prototype.toRaw = function toRaw(enc) {
 ...
 * @param {Buffer} data
 * @param {String?} enc - Encoding, can be `'hex'` or null.
 * @returns {NakedTransaction} A "naked" transaction object.
 */

Transaction._fromRaw = function _fromRaw(data, enc) {
  if (enc === 'hex')
...
Transaction.fromRaw = function fromRaw(data, enc) {
  return new bcoin.tx(Transaction._fromRaw(data, enc));
};
...
Transaction.prototype.toExtended = function toExtended(saveCoins) {
...
Transaction._fromExtended = function _fromExtended(buf, saveCoins) {
...
Transaction.fromExtended = function fromExtended(buf, saveCoins) {
  return new Transaction(Transaction._fromExtended(buf, saveCoins));
};
...
Transaction.prototype.toCoins = function toCoins() {
  return bcoin.coins.fromTransaction(this);
};

/**
 * Test an object to see if it is a Transaction.
 * @param {Object} obj
 * @returns {Boolean}
 */

Transaction.isTransaction = function isTransaction(obj) {
  return obj
