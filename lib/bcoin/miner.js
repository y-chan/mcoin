...
Miner.prototype._init = function _init() {
  ...
  if (this.mempool) {
    this.mempool.on('tx', function(tx) {
      if (!self.running)
        return;
      self.attempt.addTransaction(tx);
    });
  } else if (this.pool) {
    ...
    self.chain.computeBlockVersion(self.chain.tip, function(err, version) {
      ...
      self.mempool.getSnapshot(function(err, hashes) {
        ...
        utils.forEachSerial(hashes, function(hash, next) {
          self.mempool.getTransaction(hash, function(err, tx) {
            if (err)
              return next(err);
...
MinerBlock.prototype.addTransaction = function addTransaction(tx) {
  ...
  if (this.block.hasTransaction(tx))
    return false;
