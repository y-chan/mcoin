...
function Mempool(options) {
  ...
  this.locker = new bcoin.locker(this, this.addTransaction, 20 << 20);
...
Mempool.prototype.addBlock = function addBlock(block, callback, force) {
  ...
  utils.forEachSerial(block.txs, function(tx, next) {
    ...
    self.getTransaction(hash, function(err, existing) {
      if (err)
        return callback(err);
...
Mempool.prototype.removeBlock = function removeBlock(block, callback, force) {
  ...
  utils.forEachSerial(block.txs.slice().reverse(), function(tx, next) {
    ...
    self.hasTransaction(tx.hash('hex'), function(err, result) {
      if (err)
        return next(err);
...
Mempool.prototype.getTransaction = function getTransaction(hash, callback) {
  if (hash instanceof bcoin.tx)
    hash = hash.hash('hex');
  return this.tx.getTransaction(hash, callback);
};
...
Mempool.prototype.getTransactionsByAddress = function getTransactionsByAddress(addresses, callback) {
  return this.tx.getTransactionsByAddress(addresses, callback);
};
...
Mempool.prototype.hasTransaction = function hasTransaction(hash, callback) {
  return this.tx.hasTransaction(hash, callback);
};
...
Mempool.prototype.addTransaction = function addTransaction(tx, callback, force) {
  var self = this;
  ...
  var unlock = this._lock(addTransaction, [tx, callback], force);
  if (!unlock)
    return;
  ...
  this.chain.checkFinal(this.chain.tip, tx, lockFlags, function(err, isFinal) {
    ...
    self.seenTransaction(tx, function(err, exists) {
      if (err)
        return callback(err);
...
Mempool.prototype.seenTransaction = function seenTransaction(tx, callback) {
  var self = this;
  var hash = tx.hash('hex');
  ...
  return this.hasOrphan(hash, function(err, result) {
    ...
    return self.hasTransaction(hash, function(err, result) {
      if (err)
        return callback(err);
      ...
      return self.chain.db.hasTransaction(hash, callback);
    });
  });
};
    ...
    return isDoubleSpend(function(err, result) {
      ...
      return self.hasTransaction(hash, function(err, result) {
        ...
        function getBlock(callback) {
          ...
          return self.chain.db.getTransaction(hash, function(err, existing) {
            if (err)
              return callback(err);
...
Mempool.prototype._removeUnchecked = function removeUnchecked(hash, callback, force) {
  ...
  this.getTransaction(hash, function(err, tx) {
    ...
    utils.forEachSerial(tx.inputs, function(input, next) {
      ...
      self.hasTransaction(input.prevout.hash, function(err, result) {
        if (err)
          return next(err);
