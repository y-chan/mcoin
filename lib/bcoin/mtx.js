...
function MutableTransaction(options) {
  ...
  if (!(this instanceof MutableTransaction))
    return new MutableTransaction(options);
...
utils.inherits(MutableTransaction, bcoin.tx);
...
MutableTransaction.prototype.clone = function clone() {
  var tx = new MutableTransaction(this);
...
MutableTransaction.prototype.hash = function hash(enc) {
...
MutableTransaction.prototype.witnessHash = function witnessHash(enc) {
  var hash;
...
MutableTransaction.prototype.render = function render() {
  return this.getRaw();
};
...
MutableTransaction.prototype.renderNormal = function renderNormal() {
...
MutableTransaction.prototype.renderWitness = function renderWitness() {
...
MutableTransaction.prototype.getRaw = function getRaw() {
  if (this.hasWitness())
    return bcoin.protocol.framer.witnessTX(this);
...
MutableTransaction.prototype.getSize = function getSize() {
...
MutableTransaction.prototype.getVirtualSize = function getVirtualSize() {
 ...
 * @param {Object|Transaction|Coin} options - Options object, transaction, or coin.
 * @param {Number?} index - Input of output if `options` is a Transaction.
 */

MutableTransaction.prototype.addInput = function addInput(options, index) {
  var input;
...
MutableTransaction.prototype.scriptInput = function scriptInput(index, addr) {
...
MutableTransaction.prototype.createSignature = function createSignature(index, prev, key, type, version) {
  var hash;
...
MutableTransaction.prototype.signInput = function signInput(index, addr, type) {
...
MutableTransaction.prototype.isSigned = function isSigned() {
...
MutableTransaction.prototype.sign = function sign(index, addr, type) {
  var input;
...
MutableTransaction.prototype.addOutput = function addOutput(obj, value) {
  var options, output;
...
/**
 * Build output script (called automatically from {@link MutableTransaction#addOutput}).
 * @param {Number} index - Output index.
 * @param {Object} options - See {@link Script.createOutputScript}.
 */

MutableTransaction.prototype.scriptOutput = function scriptOutput(index, options) {
  var output;
...
MutableTransaction.prototype.isScripted = function isScripted() {
  var i, input;
...
MutableTransaction.prototype.maxSize = function maxSize(options, force) {
 ...
 * See {@link Transaction#getMinFee} vs. {@link Transaction#getMaxFee}.
...
MutableTransaction.prototype.selectCoins = function selectCoins(coins, options) {
  var chosen = [];
  var index = 0;
  var tx = this.clone();
  ...
  function addCoins() {
    while (index < coins.length) {
      // Add new inputs until Transaction will have enough
  ...
  if (options.fee) {
    ...
    // calculate maximum Transaction size.
    tx.addOutput({
 ...
 * @param {Coin[]} coins
 * @param {Object} options - See {@link MutableTransaction#selectCoins} options.
 * @returns {Object} See {@link MutableTransaction#selectCoins} return value.
 */

MutableTransaction.prototype.fill = function fill(coins, options) {
  var result, i;

  assert(this.ts === 0, 'Cannot modify a confirmed tx.');
  assert(this.inputs.length === 0, 'Transaction is already filled.');
...
MutableTransaction.prototype.sortMembers = function sortMembers() {
  var changeOutput;
...
MutableTransaction.prototype.avoidFeeSniping = function avoidFeeSniping(height) {
  if (height == null)
...
MutableTransaction.prototype.setLocktime = function setLocktime(locktime) {
  var i, input;
...
/**
 * @see Transaction._fromJSON
 */

MutableTransaction._fromJSON = bcoin.tx._fromJSON;

/**
 * @see Transaction.fromJSON
 */

MutableTransaction.fromJSON = function fromJSON(json) {
  return new MutableTransaction(MutableTransaction._fromJSON(json));
};

/**
 * @see Transaction._fromRaw
 */

MutableTransaction._fromRaw = bcoin.tx._fromRaw;

/**
 * @see Transaction.fromRaw
 */

MutableTransaction.fromRaw = function fromRaw(data, enc) {
  return new MutableTransaction(MutableTransaction._fromRaw(data, enc));
};

/**
 * @see Transaction._fromExtended
 */

MutableTransaction._fromExtended = bcoin.tx._fromExtended;

/**
 * @see Transaction.fromExtended
 */

MutableTransaction.fromExtended = function fromExtended(data, enc) {
  return new MutableTransaction(MutableTransaction._fromExtended(data, enc));
};

/**
 * Convert a Transaction to an MutableTransaction.
 * @param {Transaction} tx
 * @returns {MutableTransaction}
 */

MutableTransaction.fromTransaction = function fromTransaction(tx) {
  return new MutableTransaction(tx);
};

/**
 * Convert the MutableTransaction to a Transaction.
 * @returns {Transaction}
 */

MutableTransaction.prototype.toTransaction = function toTransaction() {
...
MutableTransaction.isMutableTransaction = function isMutableTransaction(obj) {
  return obj
