...
function TransactionDB(prefix, db, options) {
  if (!(this instanceof TransactionDB))
    return new TransactionDB(prefix, db, options);
...
utils.inherits(TransactionDB, EventEmitter);

TransactionDB.prototype._lock = function _lock(func, args, force) {
...
TransactionDB.prototype.getMap = function getMap(tx, callback) {
...
TransactionDB.prototype.mapAddresses = function mapAddresses(address, callback) {
  var self = this;
...
TransactionDB.prototype._addOrphan = function _addOrphan(key, hash, index, callback) {
  var prefix = this.prefix + '/';
  var p;
 ...
 * @property {Hash} hash - Orphan Transaction hash.
...
TransactionDB.prototype._getOrphans = function _getOrphans(key, callback) {
  var self = this;
  var prefix = this.prefix + '/';
  var p, orphans;
  ...
  this.db.get(prefix + 'o/' + key, function(err, buf) {
    ...
    utils.forEach(orphans, function(orphan, next) {
      self.getTransaction(orphan.hash, function(err, tx) {
        if (err)
          return next(err);
...
TransactionDB.prototype.add = function add(tx, callback, force) {
  var self = this;
...
TransactionDB.prototype._add = function add(tx, map, callback, force) {
  var self = this;
  var prefix = this.prefix + '/';
  ...
  this._confirm(tx, map, function(err, existing) {
    ...
    utils.forEachSerial(tx.inputs, function(input, next, i) {
      ...
      self.getCoin(input.prevout.hash, input.prevout.index, function(err, coin) {
        ...
        if (coin) {
          // Add Transaction to inputs and spend money
          input.coin = coin;
        ...
        self.isSpent(input.prevout.hash, input.prevout.index, function(err, spentBy) {
          ...
          if (spentBy) {
            return self.getTransaction(input.prevout.hash, function(err, prev) {
              if (err)
                return next(err);
 ...
 * @private
 * @param {Hash} hash
 * @param {Transaction} ref - Reference tx, the tx that double-spent.
 * @param {Function} callback - Returns [Error, Boolean].
 */

TransactionDB.prototype._removeSpenders = function removeSpenders(hash, ref, callback) {
  var self = this;
  this.getTransaction(hash, function(err, tx) {
    if (err)
      return callback(err);
...
TransactionDB.prototype.isDoubleSpend = function isDoubleSpend(tx, callback) {
  var self = this;
...
TransactionDB.prototype.isSpent = function isSpent(hash, index, callback) {
  var self = this;
  var prefix = this.prefix + '/';
...
TransactionDB.prototype._confirm = function _confirm(tx, map, callback, force) {
  var self = this;
  var prefix = this.prefix + '/';
  var hash = tx.hash('hex');
  var batch;
  ...
  this.getTransaction(hash, function(err, existing) {
    if (err)
      return callback(err);
...
TransactionDB.prototype.remove = function remove(hash, callback, force) {
  var self = this;
...
TransactionDB.prototype.lazyRemove = function lazyRemove(tx, callback, force) {
  var self = this;
...
TransactionDB.prototype._remove = function remove(tx, map, callback, force) {
  var self = this;
  var prefix = this.prefix + '/';
  var hash = tx.hash('hex');
  var batch;
...
TransactionDB.prototype.unconfirm = function unconfirm(hash, callback, force) {
  var self = this;
...
TransactionDB.prototype._unconfirm = function unconfirm(tx, map, callback, force) {
  var self = this;
  var prefix = this.prefix + '/';
...
TransactionDB.prototype.getHistoryHashes = function getHistoryHashes(address, callback) {
  var self = this;
  var prefix = this.prefix + '/';
  var txs = [];
  var iter;
...
TransactionDB.prototype.getUnconfirmedHashes = function getUnconfirmedHashes(address, callback) {
  var self = this;
  var prefix = this.prefix + '/';
  var txs = [];
  var iter;
...
TransactionDB.prototype.getCoinHashes = function getCoinHashes(address, callback) {
  var self = this;
  var prefix = this.prefix + '/';
  var coins = [];
  var iter;
...
/**
 * Get Transaction hashes by height range.
 * @param {WalletID|WalletID[]} address - By address (can be null).
 * @param {Object} options
...
TransactionDB.prototype.getHeightRangeHashes = function getHeightRangeHashes(address, options, callback) {
  var prefix = this.prefix + '/';
  var txs = [];
  var iter;
...
/**
 * Get Transaction hashes by height.
 * @param {Number} height
 * @param {Function} callback - Returns [Error, {@link Hash}[]].
 */

TransactionDB.prototype.getHeightHashes = function getHeightHashes(height, callback) {
  return this.getHeightRangeHashes({ start: height, end: height }, callback);
};

/**
 * Get Transaction hashes by timestamp range.
 * @param {WalletID|WalletID[]} address - By address (can be null).
 * @param {Object} options
 * @param {Number} options.start - Start height.
 * @param {Number} options.end - End height.
...
TransactionDB.prototype.getRangeHashes = function getRangeHashes(address, options, callback) {
  var prefix = this.prefix + '/';
  var txs = [];
  var iter;
 ...
 * @param {Number?} options.limit - Max number of records.
 * @param {Boolean?} options.reverse - Reverse order.
 * @param {Function} callback - Returns [Error, {@link Transaction}[]].
 */

TransactionDB.prototype.getRange = function getLast(address, options, callback) {
  var self = this;
  var txs = [];
  ...
  return this.getRangeHashes(address, options, function(err, hashes) {
    ...
    utils.forEachSerial(hashes, function(hash, next) {
      self.getTransaction(hash, function(err, tx) {
        if (err)
          return callback(err);
...
TransactionDB.prototype.getLast = function getLast(address, limit, callback) {
...
TransactionDB.prototype.getHistory = function getHistory(address, callback) {
  var self = this;
  var txs = [];
  ...
  return this.getHistoryHashes(address, function(err, hashes) {
    ...
    utils.forEachSerial(hashes, function(hash, next) {
      ...
      return callback(null, utils.sortTransaction(txs));
    });
  });
};
...
TransactionDB.prototype.getLastTime = function getLastTime(address, callback) {
  if (typeof address === 'function') {
    callback = address;
    address = null;
  }
...
TransactionDB.prototype.getUnconfirmed = function getUnconfirmed(address, callback) {
  var self = this;
  var txs = [];
...
TransactionDB.prototype.getCoins = function getCoins(address, callback) {
  var self = this;
  var coins = [];
...
TransactionDB.prototype.fillHistory = function fillHistory(tx, callback) {
  var self = this;
  ...
  utils.forEach(tx.inputs, function(input, next) {
    ...
    self.getTransaction(input.prevout.hash, function(err, tx) {
      if (err)
        return next(err);
...
TransactionDB.prototype.fillCoins = function fillCoins(tx, callback) {
  var self = this;
...
TransactionDB.prototype.getTransaction = function getTransaction(hash, callback) {
  var prefix = this.prefix + '/';
...
TransactionDB.prototype.hasTransaction = function hasTransaction(hash, callback) {
  return this.getTransaction(hash, function(err, tx) {
    if (err)
      return callback(err);
...
TransactionDB.prototype.getCoin = function getCoin(hash, index, callback) {
  var prefix = this.prefix + '/';
...
TransactionDB.prototype.hasCoin = function hasCoin(hash, index, callback) {
...
TransactionDB.prototype.getBalance = function getBalance(address, callback) {
...
TransactionDB.prototype.getHistoryHashesByAddress = function getHistoryHashesByAddress(address, callback) {
...
TransactionDB.prototype.getHistoryByAddress = function getHistoryByAddress(address, callback) {
...
TransactionDB.prototype.getCoinsByAddress = function getCoins(address, callback) {
...
TransactionDB.prototype.getUnconfirmedByAddress = function getUnconfirmedByAddress(address, callback) {
...
TransactionDB.prototype.getBalanceByAddress = function getBalanceByAddress(address, callback) {
...
TransactionDB.prototype.zap = function zap(address, now, age, callback, force) {
  var self = this;
