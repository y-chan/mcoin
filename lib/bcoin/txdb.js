...
TXDB.prototype.addBlock = function addBlock(block, txs, callback, force) {
  ...
  utils.forEachSerial(txs, function(tx, next) {
    ...
    self.writeTip(block.hex(), callback);
  });
};
...
TXDB.prototype._add = function add(tx, map, callback, force) {
  ...
  this._confirm(tx, map, function(err, existing) {
    ...
    utils.forEachSerial(tx.inputs, function(input, next, i) {
      ...
      self.getCoin(prevout.hex(), prevout.index, function(err, coin) {
        if (err)
          return next(err);

        key = prevout.hex() + '/' + prevout.index;
        ...
        self.isSpent(prevout.hex(), prevout.index, function(err, spent) {
          if (err)
            return next(err);
          ...
          if (spent) {
            return self.getTX(prevout.hex(), function(err, prev) {
              if (err)
                return next(err);
      ...
      utils.forEachSerial(tx.outputs, function(output, next, i) {
        ...
        self._getOrphans(key, function(err, orphans) {
          ...
          utils.forEachSerial(orphans, function(orphan, next) {
            ...
            assert(orphan.tx.inputs[orphan.index].prevout.hex() === hash);
...
TXDB.prototype.isDoubleSpend = function isDoubleSpend(tx, callback) {
  ...
  utils.everySerial(tx.inputs, function(input, next) {
    self.isSpent(input.prevout.hex(), input.prevout.index, function(err, spent) {
      if (err)
        return next(err);
...
TXDB.prototype._remove = function remove(tx, map, callback, force) {
  ...
  this.fillHistory(tx, function(err) {
    ...
    for (i = 0; i < tx.inputs.length; i++) {
      input = tx.inputs[i];
      key = input.prevout.hex() + '/' + input.prevout.index;
      address = input.getHash('hex');
...
TXDB.prototype.getCoin = function getCoin(hash, index, callback) {
  ...
  if (coin) {
    ...
    coin.hash = Buffer.isBuffer(hash) ? hash : new Buffer(hash, 'hex');
    coin.index = index;
