...
function Pool(options) {
  ...
  this.validate = {
    // 5 days scan delta for obtaining Transactions
    delta: 5 * 24 * 3600,
    // getTransaction map
    map: {}
  };
  ...
  this.chain.on('block', function(block, entry, peer) {
    ...
    if (block.type === 'merkleblock') {
      utils.forEachSerial(block.txs, function(tx, next) {
        self._handleTransaction(tx, peer, next);
      }, function(err) {
        if (err)
          return self.emit('error', err);
...
Pool.prototype._prehandleTransaction = function _prehandleTransaction(tx, peer, callback) {
...
Pool.prototype._createPeer = function _createPeer(options) {
  ...
  peer.on('tx', function(tx) {
    self._handleTransaction(tx, peer, function(err) {
      if (err)
        self.emit('error', err);
    });
  });
  ...
  peer.on('txs', function(txs) {
    ...
    for (i = 0; i < txs.length; i++) {
      ...
      if (self.markTransaction(hash, 0))
...
Pool.prototype._handleTransaction = function _handleTransaction(tx, peer, callback) {
  var self = this;
  ...
  updated = this.markTransaction(tx, 1);
  ...
  function addMempool(tx, callback) {
    ...
    self.mempool.addTransaction(tx, callback);
  }

  this._prehandleTransaction(tx, peer, function(err) {
    if (err)
      return callback(err);
...
Pool.prototype.markTransaction = function(hash, state) {
 ...
 * @param {Hash} hash - {@link Block} or {@link Transaction} hash.
 * @param {Object?} options
 * @param {Function} callback
 */
...
Pool.prototype.getTransaction = function getTransaction(hash, range, callback) {
  var self = this;
  var cbs, tx, found, delta;

  if (!this.peers.load)
    return setTimeout(this.getTransaction.bind(this, hash, range, callback), 1000);
  ...
  // Do incremental search until the Transaction is found
...
Pool.prototype.sendTransaction = function sendTransaction(tx, callback) {
...
Pool.prototype.broadcast = function broadcast(msg, callback) {
  ...
  this.peers.all.forEach(function(peer) {
    ...
    result[0].once('reject', function(payload) {
      ...
      callback(new Error('Transaction was rejected: ' + payload.reason));
    });
  });
