...
HTTPServer.prototype._init = function _init() {
  ...
  this.get('/tx/:hash', function(req, res, next, send) {
    self.node.getTransaction(req.options.hash, function(err, tx) {
      if (err)
        return next(err);
  ...
  this.get('/tx/address/:address', function(req, res, next, send) {
    self.node.getTransactionsByAddress(req.options.addresses, function(err, txs) {
      if (err)
        return next(err);
  ...
  this.post('/wallet/:id/send', function(req, res, next, send) {
    ...
    self.walletdb.get(id, passphrase, function(err, wallet) {
      ...
      wallet.createTransaction(outputs, function(err, tx) {
        if (err) {
          wallet.destroy();
          return next(err);
        }
        ...
        self.node.sendTransaction(tx, function(err) {
          if (err)
            return next(err);
  ...
  // Wallet Transactions within time range
  ...
  this.get('/wallet/:id/tx/:hash', function(req, res, next, send) {
    self.walletdb.getTransaction(req.options.hash, function(err, tx) {
      if (err)
        return next(err);
  ...
  this.post('/broadcast', function(req, res, next, send) {
    self.pool.sendTransaction(req.options.tx, function(err) {
      if (err)
        return callback(err);
