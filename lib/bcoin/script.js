...
Script.prototype.decoder = function decoder() {
  return new ScriptDecoder(this.encode());
  if (!this._decoder) {
    if (!this.raw)
      return new ScriptDecoder(this.encode());
    this._decoder = new ScriptDecoder(this.raw);
  } else {
    this._decoder.reset();
  }
...
Script.prototype.execute = function execute(stack, flags, tx, index, version) {
  ...
  if (decoder.size > constants.script.MAX_SIZE)
    throw new ScriptError('SCRIPT_SIZE');

  //ip = 0;
  //for (;;ip++) {
  ip = -1;
  while (decoder.ip !== decoder.size) {
    ip++;
    op = decoder.next();
    ...
    if (Buffer.isBuffer(op)) {
      ...
      if (stack.negate === 0) {
        if (!Script.checkMinimal2(op, op.op, flags))
...
ScriptWriter.prototype.reset = function reset() {
  this.writer = new BufferWriter();
};

ScriptWriter.prototype.render = function render() {
  return this.writer.render(true);
};

ScriptWriter.prototype.writeOp = function writeOp(op) {
  if (typeof op === 'string') {
    op = opcodes[op];
    assert(op != null, 'Bad opcode.');
  }
  this.writer.writeU8(op);
};

ScriptWriter.prototype.writeNumber = function writeNumber(num) {
  this.writeData(Script.array(num));
};

ScriptWriter.prototype.writeString = function writeString(data, enc) {
  if (typeof data === 'string')
    data = new Buffer(data, enc);
  this.writeData(data);
};

ScriptWriter.prototype.writeData = function writeData(data, op) {
  var p = this.writer;
  ...
  if (op == null) {
    if (data.length <= 0x4b)
      ...
      assert(false, 'Push data too large.');
  }
...
function WitnessWriter() {
  this.items = []
}

WitnessWriter.prototype.reset = function reset() {
  this.items.length = 0;
};

WitnessWriter.prototype.render = function render() {
  return new Witness(this.items);
};

WitnessWriter.prototype.writeOp = function writeOp(op) {
  assert(false, 'Bad opcode.');
};

WitnessWriter.prototype.writeNumber = function writeNumber(num) {
  this.writeData(Script.array(num));
};

WitnessWriter.prototype.writeString = function writeString(data, enc) {
  if (typeof data === 'string')
    data = new Buffer(data, enc);
  this.writeData(data);
};

WitnessWriter.prototype.writeData = function writeData(data, op) {
  this.items.push(data);
};


function ScriptDecoder(data) {
  this.reader = new BufferReader(data, true);
  this.size = data.length;
  this.index = 0;
  this.ip = 0;
}

ScriptDecoder.prototype.next = function next() {
  var p = this.reader;
  var op, size, data;
...
ScriptDecoder.prototype.nextRaw = function nextRaw() {
  var p = this.reader;
  var op, size, data;
  ...
  try {
    if (op >= 0x01 && op <= 0x4b) {
      p.seek(-1);
      data = p.readBytes(1 + op);
    } else if (op === opcodes.OP_PUSHDATA1) {
      size = p.readU8();
      p.seek(-2);
      data = p.readBytes(1 + size);
    } else if (op === opcodes.OP_PUSHDATA2) {
      size = p.readU16();
      p.seek(-3);
      data = p.readBytes(2 + size);
    } else if (op === opcodes.OP_PUSHDATA4) {
      size = p.readU32();
      p.seek(-5);
      data = p.readBytes(4 + size);
    }
  } catch (e) {
    this.reset();
    return -2;
...
ScriptDecoder.prototype.seek = function seek() {
  var p = this.reader;
  var op, size, data;
...
ScriptDecoder.prototype.reset = function reset() {
  this.reader.offset = 0;
  this.ip = 0;
};

ScriptDecoder.prototype.seekTo = function seekTo(index) {
  var i = -1;
  var op;
...
ScriptDecoder.prototype.get = function get(index) {
  if (this.index === index)
    return this.next();

  op = this.seekTo(index);
  if (op < 0)
    return op;
...
ScriptDecoder.prototype.getRaw = function getRaw(index) {
  if (this.index === index)
    return this.nextRaw();
...
Script.checkMinimal2 = function checkMinimal2(value, op, flags) {
  if (flags == null)
    flags = constants.flags.STANDARD_VERIFY_FLAGS;
  ...
  if (value.length <= 255)
    return op === opcodes.OP_PUSHDATA1;

  if (value.length <= 65535)
    return op === opcodes.OP_PUSHDATA2;
