 ...
 * @param {Transaction|Output} tx - Transaction or Output.
 ...
 * @see MutableTransaction#selectCoins
 * @see MutableTransaction#fill
 * @param {MutableTransaction} tx - _Must_ be a mutable transaction.
 * @param {Object?} options
...
Wallet.prototype.getTransaction = function getTransaction(hash, callback) {
  if (!this.provider)
    return callback(new Error('No wallet provider available.'));

  return this.provider.getTransaction(hash, callback);
};
 ...
 * @param {Function} callback - Returns [Error, {@link MutableTransaction}].
 */

Wallet.prototype.createTransaction = function createTransaction(options, outputs, callback) {
  var self = this;
  var tx, i;
...
/**
 * Zap stale Transactions from wallet (accesses provider).
 ...
 * @param {Function} callback - Returns [Boolean, Transaction[]].
 */
...
Wallet.prototype.scan = function scan(getByAddress, callback) {
  ...
  return this._scan({}, getByAddress, function(err, depth, txs) {
    ...
    if (self.provider && self.provider.addTransaction) {
      utils.forEachSerial(txs, function(tx, next) {
        self.addTransaction(tx, next);
      }, function(err) {
        if (err)
          return callback(err);
...
/**
 * Add a transaction to the wallets Transaction history (accesses provider).
 * @param {Transaction} tx
 * @param {Function} callback
 */

Wallet.prototype.addTransaction = function addTransaction(tx, callback) {
  if (!this.provider || !this.provider.addTransaction)
    return utils.asyncify(callback)(new Error('No transaction pool available.'));

  return this.provider.addTransaction(tx, callback);
};
