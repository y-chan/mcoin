...
Fullnode.prototype._init = function _init() {
  ...
  this.on('tx', function(tx) {
    self.walletdb.addTransaction(tx, function(err) {
      if (err)
        self.emit('error', err);
    });
  });

  // Emit events for valid blocks and Transactions.
 ...
 * bitcoind nodes).
 * @param {Transaction|MutableTransaction|Block} item
 ...
 * @example
 * node.sendTransaction(tx, callback);
 * node.sendTransaction(tx, true, callback);
 * @param {Transaction|MutableTransaction} item
 * @param {Boolean?} wait - Wait to execute callback until a node
 * requests our Transaction, rejects it, or the broadcast itself times out.
 * @param {Function} callback - Returns [{@link VerifyError}|Error].
 */

Fullnode.prototype.sendTransaction = function sendTransaction(item, wait, callback) {
  var self = this;
  ...
  this.mempool.addTransaction(item, function(err) {
    if (err)
      return callback(err);
...
Fullnode.prototype.scanWallet = function scanWallet(wallet, callback) {
  wallet.scan(this.getTransactionsByAddress.bind(this), callback);
};
...
Fullnode.prototype.getTransaction = function getTransaction(hash, callback) {
  var self = this;

  this.mempool.getTransaction(hash, function(err, tx) {
    if (err)
      return callback(err);
    ...
    self.chain.db.getTransaction(hash, function(err, tx) {
      if (err)
        return callback(err);
...
Fullnode.prototype.hasTransaction = function hasTransaction(hash, callback) {
  var self = this;

  this.mempool.hasTransaction(hash, function(err, result) {
    if (err)
      return callback(err);
    ...
    self.chain.db.hasTransaction(hash, callback);
  });
};
...
Fullnode.prototype.getTransactionsByAddress = function getTransactionsByAddress(addresses, callback) {
  var self = this;

  this.mempool.getTransactionsByAddress(addresses, function(err, mempool) {
    if (err)
      return callback(err);

    self.chain.db.getTransactionsByAddress(addresses, function(err, txs) {
      if (err)
        return callback(err);
