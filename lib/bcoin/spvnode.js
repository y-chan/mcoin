...
SPVNode.prototype.sendTransaction = function sendTransaction(item, wait, callback) {
  if (!callback) {
    callback = wait;
    wait = null;
  }
  ...
  if (!wait) {
    this.pool.sendTransaction(item);
    return utils.nextTick(callback);
  }

  return this.pool.sendTransaction(item, callback);
};
