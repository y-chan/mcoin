...
function ChainEntry(chain, options, prev) {
  ...
  this.hash = constants.ZERO_HASH;
  this.version = 1;
  this.prevBlock = constants.ZERO_HASH;
  this.merkleRoot = constants.ZERO_HASH;
  this.ts = 0;
  this.bits = 0;
  this.nonce = 0;
  this.height = -1;
...
ChainEntry.prototype.fromOptions = function fromOptions(options, prev) {
  ...
  if (options.hash) {
    if (typeof options.hash === 'string') {
      this.hash = new Buffer(options.hash, 'hex');
    } else {
  ...
  if (options.prevBlock) {
    if (typeof options.prevBlock === 'string') {
      this.prevBlock = new Buffer(options.prevBlock, 'hex');
    } else {
  ...
  if (options.merkleRoot) {
    if (typeof options.merkleRoot === 'string') {
      this.merkleRoot = new Buffer(options.merkleRoot, 'hex');
    } else {
...
ChainEntry.prototype.isGenesis = function isGenesis() {
  return this.hex() === this.network.genesis.hash;
};
...
ChainEntry.prototype.hex = function hex() {
  return this.hash.toString('hex');
};

ChainEntry.prototype.prevHex = function prevHex() {
  return this.prevBlock.toString('hex');
};
...
ChainEntry.prototype.getAncestors = function getAncestors(max, callback) {
  ...
  for (;;) {
    ...
    cached = this.chain.db.getCache(entry.prevHex());
...
ChainEntry.prototype.getPrevious = function getPrevious(callback) {
  return this.chain.db.get(this.prevHex(), callback);
};
...
ChainEntry.prototype.getNext = function getNext(callback) {
  var self = this;
  return this.chain.db.getNextHash(this.hex(), function(err, hash) {
    if (err)
      return callback(err);
...
ChainEntry.prototype.fromBlock = function fromBlock(block, prev) {
  ...
  this.prevBlock = new Buffer(block.prevBlock, 'hex');
  this.merkleRoot = new Buffer(block.merkleRoot, 'hex');
  this.ts = block.ts;
...
ChainEntry.prototype.fromRaw = function fromRaw(data) {
  ...
  this.prevBlock = p.readHash();
  this.merkleRoot = p.readHash();
  this.ts = p.readU32();
  this.bits = p.readU32();
...
ChainEntry.prototype.toJSON = function toJSON() {
  return {
    ...
    prevBlock: utils.revHex(this.prevBlock.toString('hex')),
    merkleRoot: utils.revHex(this.merkleRoot.toString('hex')),
    ts: this.ts,
    bits: this.bits,
    nonce: this.nonce,
    height: this.height,
...
ChainEntry.prototype.fromJSON = function fromJSON(json) {
  ...
  this.hash = new Buffer(utils.revHex(json.hash), 'hex');
  this.version = json.version;
  this.prevBlock = new Buffer(utils.revHex(json.prevBlock), 'hex');
  this.merkleRoot = new Buffer(utils.revHex(json.merkleRoot), 'hex');
  this.ts = json.ts;
  this.bits = json.bits;
