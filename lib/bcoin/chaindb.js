...
ChainDB.prototype.addCache = function addCache(entry) {
  this.cacheHash.set(entry.hex(), entry);
...
ChainDB.prototype.getHash = function getHash(height, callback) {
  ...
  if (entry) {
    callback = utils.asyncify(callback);
    return callback(null, entry.hex());
  }
...
ChainDB.prototype.get = function get(hash, callback) {
  ...
  return this.getEntry(hash, function(err, entry) {
    ...
    self.cacheHash.set(entry.hex(), entry);
...
ChainDB.prototype.disconnect = function disconnect(entry, callback) {
  ...
  batch.put(layout.R, entry.prevBlock);
  ...
  this.getBlock(entry.hex(), function(err, block) {
    if (err)
      return callback(err);
...
ChainDB.prototype.isMainChain = function isMainChain(hash, callback) {
  ...
  if (hash === this.chain.tip.hex() || hash === this.network.genesis.hash)
  ...
  this.get(block, function(err, entry) {
    ...
    self.getTip(function(err, tip) {
      ...
      (function next(err, tip) {
        ...
        if (utils.equal(tip.hash, entry.hash)) {
          batch.put(layout.R, tip.hash);
          return batch.write(callback);
        }
        ...
        self.removeBlock(tip.hex(), batch, function(err) {
          if (err)
            return callback(err);
...
ChainDB.prototype.connectBlock = function connectBlock(block, view, batch, callback) {
  ...
  for (i = 0; i < block.txs.length; i++) {
    ...
    for (j = 0; j < tx.inputs.length; j++) {
      ...
      if (this.options.indexAddress) {
        ...
        if (address) {
          prev = input.prevout;
          batch.del(layout.C(address, prev.hex(), prev.index));
        }
      }
  ...
  for (i = 0; i < view.length; i++) {
    ...
    if (coins.count() === 0) {
      ...
      this.coinCache.remove(coins.hex());
    } else {
      raw = coins.toRaw();
      batch.put(layout.c(coins.hash), raw);
      this.coinCache.set(coins.hex(), raw);
    }
  }
...
ChainDB.prototype.disconnectBlock = function disconnectBlock(block, batch, callback) {
  ...
  this.getUndoView(block, function(err, view) {
    ...
    for (i = block.txs.length - 1; i >= 0; i--) {
      ...
      for (j = 0; j < tx.inputs.length; j++) {
        ...
        if (self.options.indexAddress) {
          ...
          if (address) {
            prev = input.prevout;
            batch.put(layout.C(address, prev.hex(), prev.index), DUMMY);
          }
        }
      }
    ...
    for (i = 0; i < view.length; i++) {
      ...
      if (coins.count() === 0) {
        batch.del(layout.c(coins.hash));
        self.coinCache.remove(coins.hex());
      } else {
        raw = coins.toRaw();
        batch.put(layout.c(coins.hash), raw);
        self.coinCache.set(coins.hex(), raw);
      }
    }
...
ChainDB.prototype.fillCoins = function fillCoins(tx, callback) {
  ...
  utils.forEachSerial(tx.inputs, function(input, next) {
    ...
    self.getCoin(input.prevout.hex(), input.prevout.index, function(err, coin) {
      if (err)
        return callback(err);
...
ChainDB.prototype.fillHistory = function fillHistory(tx, callback) {
  ...
  utils.forEachSerial(tx.inputs, function(input, next) {
    ...
    self.getTX(input.prevout.hex(), function(err, tx) {
      if (err)
        return next(err);
