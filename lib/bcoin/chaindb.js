  ...
  this._ensureBlock(block, function(err, block) {
    ...
    block.txs.forEach(function(tx) {
      ...
      tx.inputs.forEach(function(input) {
        ...
        if (block.view.count(input.prevout.hash) === 0) {
          if (address) {
            batch.del(
              'u/a/' + address
              + '/' + input.prevout.hash);
      ...
      tx.outputs.forEach(function(output, i) {
        ...
        if (block.view.count(hash) === 1) {
          if (address)
            batch.put('u/a/' + address + '/' + hash, DUMMY);
        }
      });
    });

    block.view.toArray().forEach(function(coins) {
      if (coins.count() === 0)
        batch.del('u/t/' + coins.hash);
      else
        batch.put('u/t/' + coins.hash, coins.toRaw());
    });
...
ChainDB.prototype.getCoinView = function getCoinView(hashes, callback) {
  var self = this;
  var view = new bcoin.coinview();
  utils.forEach(hashes, function(hash, next) {
    self.getCoins(hash, function(err, coins) {
      if (err)
        return next(err);
...
ChainDB.prototype.getHistoricalCoinView = function getHistoricalCoinView(hashes, callback) {
  var self = this;
  var view = new bcoin.coinview();
  utils.forEach(hashes, function(hash, next) {
...
ChainDB.prototype.getCoins = function getCoins(hash, callback) {
  ...
  this.db.get(id, function(err, data) {
    ...
    try {
      coins = bcoin.coins.fromRaw(data, hash);
    } catch (e) {
      return callback(e);
    }
...
ChainDB.prototype.getCoin = function getCoin(hash, index, callback) {
  return this.getCoins(hash, function(err, coins) {
    if (err)
      return callback(err);
    ...
    return callback(null, coins.get(index));
  });
};
...
ChainDB.prototype._ensureBlock = function _ensureBlock(hash, callback) {
  ...
  if (hash instanceof bcoin.block) {
    if (hash.view)
      return callback(null, hash);
    return self.fillBlock(hash, callback);
  }
...
ChainDB.prototype.fillBlock = function fillBlock(block, callback) {
  var tmp = new bcoin.coinview();
  var tx, input, i, j;

  if (block.txs.length === 1) {
    block.view = tmp;
    return callback(null, block, tmp);
  }

  return this.getCoinView(block.getPrevout(), function(err, view) {
    if (err)
      return callback(err);
    ...
    for (i = 0; i < block.txs.length; i++) {
      ...
      for (j = 0; j < tx.inputs.length; j++) {
        input = tx.inputs[j];
        if (!input.coin)
          input.coin = tmp.spend(input.prevout.hash, input.prevout.index);
      }

      tmp.add(tx.toCoins());
    }

    return callback(null, block, view);
  });
};
