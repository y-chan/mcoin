...
function ConfirmStats(estimator, buckets, maxConfirms, decay, type) {
  var i;

  if (!(this instanceof ConfirmStats))
    return new ConfirmStats(estimator, buckets, maxConfirms, decay, type);

  this.estimator = estimator;
  this.logger = estimator.logger;
...
ConfirmStats.fromRaw = function fromRaw(estimator, data, type) {
  ...
  stats = new ConfirmStats(estimator, buckets, maxConfirms, decay, type);
...
function PolicyEstimator(mempool, minRelay) {
  var fee, priority, boundary;

  if (!(this instanceof PolicyEstimator))
    return new PolicyEstimator(mempool, minRelay);

  this.mempool = mempool;
  this.logger = mempool.logger;
  this.network = mempool.network;
  ...
  this.feeStats = new ConfirmStats(this,
  ...
  this.priStats = new ConfirmStats(this,
...
PolicyEstimator.fromRaw = function fromRaw(mempool, minRelay, data) {
  var p = new BufferReader(data);
  var bestHeight = p.readU32();
  var estimator = new PolicyEstimator(mempool, minRelay);
  var feeStats = ConfirmStats.fromRaw(estimator, p.readVarBytes(), 'FeeRate');
  var priStats = ConfirmStats.fromRaw(estimator, p.readVarBytes(), 'Priority');
