...
Chain.prototype._init = function _init() {
  ...
  this.on('block', function(block, entry) {
    ...
    self.logger.info('Block %s (%d) added to chain.',
      utils.revHex(entry.hex()), entry.height);
  });
  ...
  this.on('competitor', function(block, entry) {
    self.logger.warning('Heads up: Competing chain at height %d:'
      ...
      self.tip.height,
      entry.height,
      utils.revHex(self.tip.hex()),
      utils.revHex(entry.hex()),
  ...
  this.db.getChainHeight(function(err, chainHeight) {
    ...
    stream.on('data', function(data) {
      ...
      for (i = 0; i < blocks.length; i++) {
        ...
        if (lastEntry && block.prevBlock !== lastEntry.hex()) {
          stream.destroy();
...
Chain.prototype.checkDuplicates = function checkDuplicates(block, prev, callback) {
  ...
  this.db.get(this.network.block.bip34height, function(err, entry) {
    ...
    if (entry && entry.hex() === self.network.block.bip34hash)
      return callback();
...
Chain.prototype.findFork = function findFork(fork, longer, callback) {
  (function find() {
    if (utils.equal(fork.hash, longer.hash))
  ...
  return this.findFork(tip, entry, function(err, fork) {
    ...
    function disconnect(callback) {
      ...
      (function collect(entry) {
        if (utils.equal(entry.hash, fork.hash))
          return finish();
    ...
    return disconnect(function(err) {
      ...
      return reconnect(function(err) {
        ...
        self.emit('reorganize', block, tip.height, tip.hex());
...
Chain.prototype.reconnect = function reconnect(entry, callback) {
  ...
  this.db.getBlock(entry.hex(), function(err, block) {
    if (err)
      return callback(err);
...
Chain.prototype.setBestChain = function setBestChain(entry, block, prev, callback) {
  ...
  if (!this.tip) {
    if (entry.hex() !== this.network.genesis.hash) {
  ...
  if (utils.equal(entry.prevBlock, this.tip.hash))
    return done();
...
Chain.prototype.getTargetAsync = function getTargetAsync(block, prev, callback) {
  ...
  return prev.getAncestors(this.network.pow.retargetInterval, function(err, ancestors) {
    if (err)
      return callback(err);
...
Chain.prototype.getState = function getState(prev, id, callback) {
  ...
  (function walk(err, entry) {
    ...
    if (self.stateCache[entry.hex()] != null)
      return walkForward(self.stateCache[entry.hex()]);
  ...
  function walkForward(state) {
    ...
    switch (state) {
      case constants.thresholdStates.DEFINED:
        return entry.getMedianTimeAsync(function(err, medianTime) {
          ...
          if (medianTime >= timeTimeout) {
            self.stateCache[entry.hex()] = constants.thresholdStates.FAILED;
            return walkForward(constants.thresholdStates.FAILED);
          }

          if (medianTime >= timeStart) {
            self.stateCache[entry.hex()] = constants.thresholdStates.STARTED;
            return walkForward(constants.thresholdStates.STARTED);
          }

          self.stateCache[entry.hex()] = state;
      ...
      case constants.thresholdStates.STARTED:
        return entry.getMedianTimeAsync(function(err, medianTime) {
          ...
          function doneCounting(err) {
            ...
            if (count >= threshold) {
              self.stateCache[entry.hex()] = constants.thresholdStates.LOCKED_IN;
      ...
      case constants.thresholdStates.LOCKED_IN:
        self.stateCache[entry.hex()] = constants.thresholdStates.ACTIVE;
