...
Chain.prototype._init = function _init() {
  ...
  this.on('block', function(block, entry) {
    ...
    self.logger.debug('Block %s (%d) added to chain.',
  ...
  this.on('resolved', function(block, entry) {
    self.logger.debug('Orphan %s (%d) was resolved.', block.rhash, entry.height);
  });

  this.on('checkpoint', function(block, height) {
    self.logger.debug('Hit checkpoint block %s (%d).', block.rhash, height);
  });
  ...
  this.on('invalid', function(block, height) {
    self.logger.warning('Invalid block at height %d: hash=%s', height, block.rhash);
  });
  ...
  this.on('purge', function(count, size) {
    self.logger.debug('Warning: %d (%dmb) orphans cleared!', count, utils.mb(size));
  });
  ...
  this.db.getChainHeight(function(err, chainHeight) {
    ...
    stream.on('data', function(data) {
      ...
      for (i = 0; i < blocks.length; i++) {
        ...
        if ((height + 1) % 50000 === 0)
          self.logger.info('Received %d headers from electrum.org.', height + 1);
